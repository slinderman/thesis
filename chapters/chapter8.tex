...
\end{align*}
where each term,~$\phi_n(\omega \given b)$, is nonnegative, and~$\phi_0(\omega \given b)=1$.
    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}
        ...
        \While {not accept}
          ...
          \For {$n=1$ to~$\infty$}
            \State $\Phi \from \Phi + (-1)^n \phi_n(\omega, b)$ \Comment{Update partial sum}
            \If {$\phi_n < \phi_{n-1}$}  \Comment{Check if terms are decreasing}
              \If {$n$ odd and $u \leq \Phi$}  \Comment{Compare to lower bound}
                \State accept $\from$ True  \Comment{Accept and return}
                \State \textbf{break}
                \EndIf
              \If {$n$ even and~$u > \Phi$}  \Comment{Compare to upper bound}
