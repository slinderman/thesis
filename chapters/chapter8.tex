    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}
        ...
        \While {not accept}
          ...
          \State $u \sim \distUniform(0,1)$
          \State $\phi_0 = 1$
          \State $\Phi = \phi_0$
          \For {$n=1$ to~$\infty$}
            \State $\Phi \from \Phi + \phi_n$
            \If {$\phi_n < \phi_{n-1}$}
              \If {$n$ odd and $u < \Phi$}
                \State accept $\from$ True
                \State break
                \EndIf
              \If {$n$ even and~$u > \Phi$}
                \State break
      ...
      \end{algorithmic}
      \caption{A rejection sampling algorithm for the \polyagamma distribution in the ``small-shape'' regime.}
