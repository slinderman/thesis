...
\section{A Hierarchy of Latent State Space Models}
...
Now consider the special case where there are only~$K<T$ unique dynamics
  ...
  \begin{minipage}{.75\textwidth}
    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}
        ...
        \EndFor
      \end{algorithmic}
      \caption{Forward filtering-backward sampling (FFBS) algorithm for the Gaussian linear dynamical system. Note the similarity to the FFBS algorithm for HMMs in Alg.~\ref{alg:ffbs_hmm}.}
...
parameters of an SLDS is summarized in Algorithm~\ref{alg:slds}.
  ...
  \begin{minipage}{.75\textwidth}
    ...
    \begin{algorithm}[H]
      ...
      \begin{algorithmic}
        ...
        \Comment{Algorithm~\ref{alg:ffbs_hmm}}
...
Sometimes we only have partial observations. For example, in some
cases we have multiple recordings from the same circuit, but each
recording only provides access to a subset of the population of
neurons \citep{turaga2013inferring}. In other cases, we simply hold
out some of the data for predictive likelihood comparisons. With
Gaussian observations, we can implement this by replacing the missing
data point,~$s_{t,n}$, with a zero mean, zero precision
observation. In the discrete count model, this can be implemented by
setting the auxiliary variable,~$\omega_{t,n}$, and the effective
observation,~$\widehat{s}_{t,n}$, to zero. Recall that the \polyagamma
auxiliary variables specify the precision of the effective
observations. By setting this to zero, we effectively remove this data
point.
...
% This sequence of geometric averages is analogous to progressively lowering the 
% ``temperature'' of the likelihood. We start with a high temperature (high entropy) prior 
% and eventually cool the distribution until we arrive at the low entropy posterior.
% Hence the analogy with annealing. From a statistical perspective, when the likelihood
% is a product of exponential family densities, it collapses into a density parameterized 
% by a sufficient statistic, that raising it to a power less than one is akin to reducing 


In addition to a annealing path, we also need a sequence of MCMC
transition operators that leave the intermediate distributions~$q_m$
invariant,
\begin{align*}
  \mcT_m(\btheta, \bz, \bx \to \btheta', \bz', \bx').
\end{align*}
Starting with a sample from the prior and applying these
transition operators for~${m=1, \ldots, M}$ yields a sample that is closer
in distribution to the posterior. AIS uses this procedure as a
proposal distribution for importance sampling.  The importance weights
are given by a product of ratios between~$f_m$ and~$f_{m-1}$.  Since
the target density is the unnormalized posterior density, the
importance weights will be unbiased estimates of the normalization
constant, namely the marginal likelihood,~${\mcZ_M =p(\bs)}$. The
annealed importance sampling algorithm is summarized in
Algorithm~\ref{alg:ais}.
\begin{center}
...
effective, in that they reach the equilibrium distribution more quickly.
...
\end{align*}
When~$\psi=0$, the inverse Gaussian
...
Figure~\ref{fig:pg_phi} plots~$\Phi(\pgvar \given b)$ for various
...
%    &= 1 + \frac{2(n+1)b +b^2}{2n^2+nb+2n+b} \\
...
Algorithm~\ref{alg:pg_sampling} provides pseudocode for the
final rejection sampling algorithm.

%Empirically, this rarely takes more than six iterations.
...
\begin{center}
  \begin{minipage}{.85\textwidth}
    ...
    \begin{algorithm}[H]
      \sffamily
      \begin{algorithmic}
        \Require $b > 0$,~$\psi \in \reals$
        \State accept $\from$ False
        \While {not accept}
          \State $\omega \sim \distInvGaussian \left(\frac{b}{|\psi/2|}, b^2 \right)$ \Comment{Inv. Gaussian proposal}
          \State $u \sim \distUniform(0,1)$ \Comment{Sample acceptance variable}
          \State $\Phi = 1$  \Comment{Initialize partial sum with first term}
          \For {$n=1$ to~$\infty$}
            \State $\Phi \from \Phi + (-1)^n \phi_n(\omega \given b)$ \Comment{Update partial sum}
            \If {$\phi_n(\omega \given b) < \phi_{n-1}(\omega \given b)$}  \Comment{Check if terms are decreasing}
              \If {$n$ odd \textbf{and} $u \leq \Phi$}  \Comment{Compare to lower bound}
                \State accept $\from$ True  \Comment{Accept and return}
                \State \textbf{break}
                \EndIf
              \If {$n$ even \textbf{and}~$u > \Phi$}  \Comment{Compare to upper bound}
                \State \textbf{break}  \Comment{Reject and make new proposal}
              \EndIf
            \EndIf
          \EndFor
        \EndWhile
      ...
      \end{algorithmic}
      \caption{A rejection sampling algorithm for the \polyagamma distribution that is most efficient in the ``small-shape'' ($b < 1$) regime.}
      \label{alg:pg_sampling}
    \end{algorithm}
    \vspace{.5em}
  \end{minipage}
\end{center}
...
% of real world modeling problems. 
% For now, however, 
