...
nonstationary, dynamics at time~$t$ governed by~$\bA_t$,~$\bb_t$, and
...
nonlinear function of~$\psi_{t,n}$ and neuron-specific
...
\end{align*}
Thus, for this transition to be efficient, we must be able to sample
from the \polyagamma conditional distribution in the regime
where~${b(s_{t,n}, \nu_n) \cdot \beta_m < 1}$.  For Bernoulli
observations,~${b(s_{t,n}, \nu_n) \equiv 1}$, so we will be in this
regime for all~$\beta_m \in [0,1)$.  While efficient samplers exist
  ...
  greater than or equal to one \citep{windle2014sampling}, the default
  method for this ``small shape'' is to approximate a \polyagamma
  sample with a weighted sum of gamma random variates
  ...
  draw, we typically need to sample around 200 gamma random variates
  per \polyagamma sample.  With~$TN$ auxiliary variables, this quickly
