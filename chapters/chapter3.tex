...
For example, if our signal is a spike train with at most one spike 
per time bin, then a natural model is,
\begin{align}
  p(s_{t,n} \given \psi_{t,n}) &\sim \distBernoulli(s_{t,n} \given \sigma(\psi_{t,n})),
\end{align}
where~${\sigma(s)=e^s / (1+e^s)}$ is the logistic function that maps 
the real line to the interval~${[0,1]}$. If we are modeling real-valued calcium 
fluorescence, a more natural model is,
\begin{align}
  p(s_{t,n} \given \psi_{t,n}, \nu^2) &\sim \distNormal(s_{t,n} \given \psi_{t,n}, \nu^2),
\end{align}
where~$\nu^2$ is the variance of the observed signal. In both cases, 
the activation captures the underlying signal that determines 
the mean of the observations.

Our goal is to model the temporal dynamics of our observed signal 
...
activation at time~$t$ and the activation at times~$[0,t-1]$,
\begin{align}
...
We typically assume that the basis functions and the signal 
are fixed so that we can precompute the inner sum, which is simply the 
convolution of the signal with the basis function.
Then, we can write the activation in a simplified matrix form,
\begin{align}
  \bpsi_{t} &\equiv \bb  + \sum_{k=1}^K \sum_{m=0}^N W_{n,m,k} \, \widehat{s}_{t,m,k} \\
             &= \bb + \bW \, \widehat{\bs}_t,
\end{align}
where~$\bb \in \reals^N$,~$\bW \in \reals^{N \times NK}$,
and~$\widehat{\bs}_t \in \reals^{NK}$.

Suppose, for example, that there is one basis function ($K=1$)
defined by,~${\phi_{1,\Delta t} = e^{-\Delta t/\tau}}$.
Then~$\widehat{s}_{t,m,1}$ is a sum of spikes in the window~$[t-\Delta t_{\mathsf{max}},t-1]$,
weighted by an exponential function with time constant~$\tau$.
If the ``weight,''~$W_{n,m}$, is positive, the influence will be
excitatory, and if it is negative, the effect will be inhibitory.
The GLM corresponds to an autoregressive model of spike counts 
in which the link between previous spike counts and the current 
spike count takes the form of a linear-nonlinear cascade.

Our goal is to discover the weights,~$\bW$, which are naturally 
thought of as a ``functional'' network relating the activity of 
one neuron to the future activity of others. 
While it is tempting to relate these 
weights to underlying, synaptic connections between neurons, 
in general this is unwarranted. The weights of the generalized 
linear model can reflect a number of latent factors, like 
polysynaptic connections and common inputs. Nevertheless, 
these functional networks can still contain valuable 
structure that is useful for understanding the neural population. 
...
\begin{align}
p(\bs, \bA, \bW, \bz, \theta) 
%&= p(\theta) \, p(\bz \given \theta) \, p(\bA, \bW \given \bz, \theta) \, p(\bs, \bpsi \given \bA, \bW) \\
&= 
