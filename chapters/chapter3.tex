...
Thus, latent structure is defined in terms of patterns of interaction rather than patterns of activity. 
...
We demonstrate the efficacy of this approach on simultaneously recorded multi-neuronal spike trains and synthetic data.
...
\begin{figure}[t!]
  ...
  \\
  \vspace{-.1in}
  \caption[Examples of network models]{Example network models.
    Each row corresponds to a fixed weight matrix,~$\bW$, 
...
\TODO{update this when we have all the results}
...
latent variables, one may instead fit the network with an $L_1$-regularized
GLM and \textit{then} fit a probabilistic network model to the GLM
connection weights. This is the approach taken by~\citet{stevenson2009bayesian}.
The advantage is that the network is only fit once, and since this
is usually the most expensive aspect of inference, it can save considerable
time. However, when the data
is limited, both the network and the latent variables are uncertain.
Our Bayesian approach finds joint assignments with high posterior probability,
whereas the standard GLM finds a single network that does not account for
prior beliefs about structure underlying the network.
In this example, subsequently fitting a latent distance model to the
adjacency matrix of a thresholded GLM network finds an embedding
that is somewhat like the true locations, but is vastly inferior to
the embedding found by our Bayesian approach. This is illustrated
by the increased correlation between true and inferred pairwise
distances under the joint, Bayesian approach, as shown in 
...
\begin{figure}[t!]
  ...
  \caption[Inferred connection probabilities for synthetic RGC data]{
    ...
    \textbf{(b)} Inferred connection probability found by our fully Bayesian approach.
    \textbf{(c)} Inferred connection probability found by fitting a latent distance model to the thresholded network from the MAP estimate of an~$L_1$-regularized GLM.
  }
...
give rise to the distance-dependent connection probabilities.  By
...
breakdown the wall-clock time into time spent updating the auxiliary
...
For each potential connection, the cost per iteration can also depend
on~$N$ in the worst case. Fortunately, the complexity of our collapsed
Gibbs sampling algorithm (described in Section~\ref{sec:methods}) only
depends on the number of incident connections received by each neuron,~$d$.
Specifically, we must invert a~$d \times d$ matrix, which incurs an~$O(d^3)$
cost. Figure~\ref{fig:runtime_vs_rho} illustrates this for a Bernoulli network
with~$N=200$ neurons. The expected number of incoming connections
is related to the probability of connection by~$\bbE[d]=\rho N$. If we
increased the number of neurons but kept the average in-degree constant,
the total cost would scale only quadratically with~$N$.
...
structure of neural populations.

\TODO{Lead in...}


\subsection{Approximate Methods}
For large networks, the bulk of the running time is spent 
...
to spike trains does not yield meaningful features. These
...
lies in the pattern of sparsity in the functional network. 

We may, however, resort to approximate methods that only estimate the
functional network once, rather than resampling it every
iteration. Conditioned on this network, we can attempt to learn its
underly structure. This is the approach taken
by~\citet{stevenson2009bayesian}. The simplest way to approximate the
network is by summing the cross-correlation of the spike trains for
a sequence of time lags. A more accurate and only slightly more expensive
approach is to fit a standard GLM to the spike train using a
sparsity-inducing regularization penalty. As the number of time bins
in our dataset grows, the standard GLM will yield networks that
approach the networks learned via our fully Bayesian method, since the
effect of the prior is limited in the large-data regime. However, in
modern recordings the number of time bins and the number of neurons
are often comparable, in which case a well-informed prior can
substantially improve inference results.

While these methods yield matrices of functional weights, we must
then threshold these matrices to obtain a sparse functional network.
For cross-correlation, there is no clear way to set this threshold,
so we must instead try multiple values. A standard GLM with
an $L_1$ penalty will yield networks with weights that are exactly
zero. However, we must then set the regularization parameter that
governs this threshold. In practice, this requires fitting multiple
networks and evaluating them with cross-validation. Eventually,
the cost of this approach becomes comparable to that of our
