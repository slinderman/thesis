...
Section~\ref{sec:motifs}, different patterns of conditional dependence
that provide the building blocks of models for dynamic data.
...
~${\distDirichlet(\bpi \given \gamma)}$, is similarly conjugate
with the product-of-Categorical likelihood that links~$\bpi$ and~$\bz$.
In fact,
...
state~$z_2$ and the firing rates,~$\blambda$.  Thus, the oint
...
over and over again in time series modeling, and form the foundational
building blocks for more complex models. Figure~\ref{fig:motifs}
shows graph structures corresponding to a few of these common motifs.
  \todo{change notation in figure}
...
This is known as a hidden Markov model (HMM), and in
Chapter~\ref{chap:hmms} we will study some of the challenges involved
in selecting the number of states,~$K$, in a nonparametric way.
...
autoregresive model for latent states, here the
...
the past~$D$ time bins, allowing delayed interactions.  Since the
firing rates must be non-negative, the weights must be as well.  That
is,~$w_{n \from m}^{(d)} \in \reals_+$.

By constraining the weights to be non-negative, we are instantiating
...
linear autoregressive dynamics in the latent state.  Replace the prior
on~$\bx$ with a model of the form,
\begin{align*}
...
over next states remains unchanged. When~$\bz$ and~$\btheta$ are
discrete, the stationary distribution is an eigenvector of 
a transition matrix with eigenvalue one. 
...
\item \textsc{Block Gibbs Sampling}:
  ...
  distribution over the latent state,~$z_t$, in an HMM, given all other
  variables,  
  \begin{align*}
    p(z_t \given \bs, \bz_{\neg t}, \btheta) 
  ...
  state~$z_t$ may be essentially deterministic. Thus, even if there is
...
\footnote{Since~$q$ is not strictly a conditional distribution, we use a
  semicolon to indicate that it is parameterized by~$\bvartheta$.}
