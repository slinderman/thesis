...
Neuroscience is undergoing a technological revolution.  In laboratories around
the world, optical tools are illuminating thousands of neurons at a
time and silicon probes are providing high-fidelity recordings at
similar scales.  For some organisms, we can now monitor the activity of
every single neuron in the brain.  Not long ago, this immense
recording capability would have been mind-boggling to the average
practitioner poking around in the dark, hoping to record from a
handful of neurons.  Complementing these recording capabilities,
recent advances in connectomics --- the mapping of synaptic
connectivity --- are providing a more complete atlas of the wiring
diagram that underlies neural activity.  Likewise, sophisticated
tracking and processing algorithms are providing rich descriptions of
the natural behavior that this activity gives rise to.  These
unprecedented technological capabilities are leading to a fundamental
paradigm shift.
...
measured data, and revising them accordingly, is becoming increasingly
...
specify what types of structure to look for, and in doing this we are
...
with a fundamental challenge: how do we formulate models that capture
our intutitions and hypotheses about neural computation, and how do we
fit these models to large scale recordings?

This thesis builds a sequence of modeling and inference tools to
address the problem. We build tools for analyzing \emph{neural spike
...
the roughly 170 billion cells in our brain are neurons, the
...
potassium, and calcium, which together maintain a difference in
electrical potential between the inside and the outside of the cell
membrane. When the membrane potential is excited above a certain
threshold, a cascade of ion channels opens and closes, causing the
membrane potential to undergo a characteristic action potential, or
``spike.''
...
open and allowing current to flow into or out of the post-synaptic
cell. These currents induce brief changes in the membrane potential of
the downstream cell called post-synaptic potentials (PSPs). Depending
...
manifests as changes in the amplitude of PSPs. This synaptic
...
Still, these model organisms have taught us a great deal about the workings
of human brains and intelligence. 
...
Ram\'on y Cajal, the ``father of modern neuroscience,'' it has been
clear that the brain is made up of anatomically distinct cell types
\citep{cajal1899textura}, but further classifications have been made
on the basis of functional distinctions as
well. \citet{kuffler1953discharge} identified the response properties
of ``on'' and ``off'' ganglion cells in the retina, characterizing the
ways in which these cells respond to light. His work which was carried
on by his students, Hubel and Wiesel, who characterized simple and
complex cells in visual cortex \citep{hubel1962receptive}, and
eventually won the Nobel Prize for their work.  These are pioneering
examples of a common theme in neuroscience: clustering cells into
discrete types that facilitate our understanding of neural
computation. Indeed, the clustering of cells in the early visual
pathway continues to this day \citep{macosko2015highly,
  sanes2015types}.
...
We often use networks to represent the dynamics of systems.
For example, an edge in the network may represent the influence that activity
...
latent variable models for the nodes with dynamics models for its edges, 
...
are naturally formalized in terms of probabilistic models, which
provide an intuitive way of expressing the joint probability of
...
spike train, and use this posterior distribution to gain insight into
the structure of the data and potential shortcomings of our theories.
...
Nevertheless, performing Bayesian inference in sophisticated models is
still a challenge. The art of probabilistic modeling lies in
balancing two objectives: designing models that capture the
details of the generative processes underlying our
data, while simultaneously ensuring that these models admit efficient
inference algorithms. To achieve this balance, we often look for
model-specific structure that we can exploit during inference. 
This thesis is largely dedicated to developing
algorithms that capitalize on the properties of models in order to
efficiently scale to large scale recordings.
...
can explain the data. We can also criticize the model, formally, by
assessing goodness-of-fit, evaluating predictive likelihoods, and
performing posterior predictive checks \citep{Gelman13}. This leads to
new hypotheses, which in turn lead to new models and repetition of
this process.  Thus, model building is an interative process in which
we are constantly adapting and refining hypotheses.
\citet{blei2014build}, from whom this figure has been adapted, calls
this ``Box's loop,'' a reference to the foundational work
of~\citet{box1980sampling}.
...
Bayesian inference algorithms that take advantage of clever data
augmentation strategies. Together, these expand the repertoire of
probabilistic models that we leverage when looking for structure in
neural data.
...
model of the chapter, we provide a more thorough discussion of network
...
domains other than neuroscience, specifically, from social network
analysis and criminology.
...
be learned directly from the data. A preliminary version of this work
...
considers dynamical state space models, specifically Hidden Markov
