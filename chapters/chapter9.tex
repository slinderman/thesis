...
While this stochastic representation may have the correct expectation,
we would like to characterize the probability that it is ``close'' to
its mean. As we hypothesized above, the difference between the true
probability and that represented by the population should shrink as
the number of spikes grows. We quantify this with an upper bound on
the number of spikes required to guarantee that the represented
probability differs from the true probability by more
than~$\epsilon$. We measure this difference with the total variation
distance between the distributions,
\begin{align*}
  \dtv(\widehat{\bpi}_t, \bpi) &= 
  \frac{1}{2} \sum_k |\widehat{\pi}_{t,k} - \pi_{k}|.
\end{align*}

In order to derive a bound on the number of spikes, we appeal to 
Lemma 3 of \citet{devroye1983equivalence}, restated here for convenience.

\begin{lemma}[Multinomial Convergence \citep{devroye1983equivalence}]
  \label{lem:devroye}
  Let~$\bx \sim \distMultinomial(N, \bpi)$ be a multinomial
  distributed vector in~$\naturals^K$ with normalized probability
  vector~$\bpi \in [0,1]^K$.  For all~$\epsilon \in (0,1)$ and all~$K$
  satisfying~$\tfrac{K}{N} \leq \tfrac{\epsilon^2}{20}$, we have,
  \begin{align*}
    \Pr \left( \frac{1}{N} \sum_{k=1}^K \big| x_k - \bbE[x_k] \big|
    > \epsilon \right)
    &\leq 3 \exp \left\{-\frac{N \epsilon^2}{25} \right\}.
  \end{align*}
\end{lemma}
...
\begin{corollary}
  \label{cor:mult_dtv}
    Let~$\bx \sim \distMultinomial(N, \bpi)$ be a multinomial
    distributed vector in~$\naturals^K$ with normalized probability
    vector~$\bpi \in [0,1]^K$, and then let~$\widehat{\bpi} =
    \tfrac{1}{N} \bx$.  For all~$\varepsilon \in (0,\tfrac{1}{2})$ and all~$K$
    satisfying~$\tfrac{K}{N} \leq \tfrac{\varepsilon^2}{5}$, we have,
  \begin{align*}
    \Pr \left( \dtv(\widehat{\bpi}, \bpi) > \varepsilon \right)
    &\leq 3 \exp \left\{-\frac{4 N \varepsilon^2}{25} \right\}.
  \end{align*}
  % TODO Delete this
  %Let~$u = \epsilon/2$ such that~$\epsilon^2 = 4u^2$. Then,
  %\begin{align*}
  %  \Pr \left( \dtv(\widehat{\bpi}, \bpi) > u \right)
  %  &\leq 3 \exp \left\{-\frac{4 N u^2 }{25} \right\}.
  %\end{align*}

\end{corollary}

\begin{proof}
  Since~$\widehat{\pi}_k = \tfrac{x_k}{N}$ and~$\tfrac{1}{N} \bbE[x_k] = \pi_k$,
  \begin{align*}
    \frac{1}{N} \sum_{k=1}^K \big| x_k - \bbE[x_k] \big|
    &= \sum_{k=1}^K \big| \widehat{\pi}_k - \pi_k \big|
    = 2\dtv (\widehat{\bpi}, \bpi).
  \end{align*}
  The statement follows from Lemma~\ref{lem:devroye} by
  substituting~$\varepsilon = 2 \epsilon$.
\end{proof}
...
time window, a fixed error level~$\epsilon < \tfrac{1}{2}$, and a desired
confidence~$\delta < 1$, the conditional probability of error is
at most~$\delta$ if,
\begin{align*}
  S_t \geq \frac{5}{\epsilon^2}
  \max \left(K, \, \frac{5}{4}\ln \frac{3}{\delta} \right).
\end{align*}
\end{theorem}

\begin{proof}
  For a given population spike count,~$S_t$, the vector $\bx_t =
  \left[ S_{t,1}, \ldots, S_{t,K} \right]$ follows a multinomial
  distribution,~$\bx_t \sim \distMultinomial(S_t, \bpi)$.
  Moreover,~$\widehat{\bpi}_t = \frac{\bx_t}{S_t}$.  From
  Corollary~\ref{cor:mult_dtv}, if~$S_t \geq \frac{5K}{\epsilon^2}$,
  the probability of the total variation distance exceeding~$\epsilon$
  is bounded by,
  \begin{align*}
    \Pr(\dtv(\widehat{\bpi}_t, \bpi) > \epsilon)
    \leq 3 \exp \left\{ -\frac{4S_t \epsilon^2}{25} \right\}.
  \end{align*}
  Setting this probability equal to~$\delta$ yields an upper bound on the
  number of spikes necessary to ensure the desired error,
  \begin{align*}
    S^* \leq \frac{25}{4\epsilon^2} \ln \frac{3}{\delta}.
  \end{align*}
  Thus, the probability of total variation distance greater than~$\epsilon$
  is bounded by~$\delta$ if,
  \begin{align*}
    S_t \geq \frac{5}{\epsilon^2}
    \max \left(K, \, \frac{5}{4}\ln \frac{3}{\delta} \right).
  \end{align*}
...
necessary to guarantee that the total variation distance between the
...
is~$\bbE[S_t] = \lambdamax T_I R$: the expected number of spikes per
neuron times the number of neurons per outcome. This leads to the
following theorem, which specifies a upper bound on the gain and
number of neurons required to guarantee that the total variation
distance is less than~$\epsilon$ with probability~$1-\delta$.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    %&\leq \sum_{m=0}^{\lceil{5K/\epsilon^2} \rceil} \Pr(S_t=m)
    &\leq \Pr(S_t < \lceil{5K/\epsilon^2} \rceil)
    + \sum_{m=0}^\infty \Pr(S_t=m) \times 3 \exp \left\{ - \frac{4S_t \epsilon^2}{25} \right\}\\
    &\leq \Pr(S_t < \lceil{5K/\epsilon^2} \rceil) + 3\bbE_{S_t}
    \left[ \exp \left\{ - \frac{4S_t \epsilon^2}{25} \right\} \right] \\
    &\leq \Pr(S_t < \lceil{5K/\epsilon^2} \rceil)
    + 3 \exp \left \{ \mu^* \left( e^{-4\epsilon^2/25} -1 \right) \right\},
  \end{align*}
  where the last line follows from moment generating function of~${S_t \sim \distPoisson(\mu^*)}$.

  \TODO{Now bound the cumulative distribution of a Poisson...}
\end{proof}
