...
Here we extend the preceding work and consider a \emph{Bayesian nonparametric} 
...
The \emph{prediction} step involves a marginalization over the
previous latent state,~$z_{t-1}$,
\begin{align*}
  p(z_t \given \bs_{1:t-1} \btheta)
  &\propto  \sum_{k=1}^K p(z_t \given z_{t-1}=k, \btheta) \, 
    p(z_{t-1}=k \given \bs_{1:t-1}, \btheta) \\
  &= \distDiscrete(z_t \given \bbm_{t|t-1}),
\end{align*}
where
\begin{align*}
  m_{t|t-1, k} &\propto \sum_{j=1}^K p(z_t=k \given z_{t-1}=j, \btheta) \cdot m_{t-1,j}
\end{align*}
Then, we \emph{condition} on the current observations,~$\bs_t$, to get the
parameters of the filtered distribution,
\begin{align}
  \label{eq:fwd_filter_hmm}
  m_{t,k} &\propto p(\bs_t \given z_t=k, \btheta) \cdot m_{t|t-1,k}.
\end{align}

Once we have computed the filtered distributions for all
time bins, we can sample from the joint distribution over~$\bz_{1:T}$
by applying the chain rule,
\begin{align*}
  p(\bz_{1:T} \given \bs_{1:T}, \btheta)
  &= p(z_T \given \bs_{1:T}, \btheta)
  \prod_{t} p(z_{t} \given \bz_{t+1:T}, \bs_{1:T}, \btheta) \\
  &\propto p(z_T \given \bs_{1:T}, \btheta)
  \prod_{t} p(z_t \given \bs_{1:t}, \btheta) \, 
    p(z_{t+1} \given z_{t}, \btheta).
\end{align*}
Thus, we can sample in reverse order, starting with~$z_T$ and ending
with~$z_1$. The conditional distribution of~${z_t}$ is,
\begin{align}
  p(z_t \given \bz_{t+1:T}, \bs_{1:T}, \btheta)
  &\propto
  p(z_t \given \bbm_t) \,
  p(z_{t+1} \given z_t, \btheta),
    \label{eq:bkwd_sample_hmm}
\end{align}
which is another discrete distribution. The final algorithm for block
Gibbs sampling~$\bz_{1:T}$ is:
...
\begin{center}
  \begin{minipage}{.75\textwidth}
    ...
    \begin{algorithm}[H]
      \sffamily
      \begin{algorithmic}
        \Require $\bs_{1:T}$, $\btheta$ 
        \For {$t = 1, \ldots, T$}
          \State Compute~$\bbm_t$ \Comment{Eq.~\ref{eq:fwd_filter_hmm}}
        \EndFor
        \For {$t = T, \ldots, 1$}
          \State Sample~$z_t \given z_{t+1}, \bbm_t, \btheta$ \Comment{Eq.~\ref{eq:bkwd_sample_hmm}}
        \EndFor
      \end{algorithmic}
      \caption{Forward filtering, backward sampling algorithm for the hidden Markov model.}
      \label{alg:ffbs_hmm}
    \end{algorithm}
  \end{minipage}
\end{center}
...
% END NEW STUFF
A single iteration of the comple Gibbs sampling algorithm consists
