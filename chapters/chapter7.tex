...
multiplicative factor. Leveraging the fact that~$\pi_{t,k} \leq 1$, we
have,
% \begin{align}
...
\begin{align}
  \Pr \left(S_{t,k} > \bbE[S_{t,k} \given S^*] \left(1+\frac{\epsilon}{\pi_{t,k}} \right) \right) 
  &\leq \exp \left \{-2S_t \epsilon^2 \right \}, \\
  \Pr \left(S_{t,k} < \bbE[S_{t,k} \given S^*] \left(1-\frac{\epsilon}{\pi_{t,k}} \right) \right) 
  &\leq \exp \left \{-2S_t \epsilon^2 \right \},
\end{align}
...
\begin{assumption}
  ...
  Thus, neuron~$n$'s estimate of~$\bpi_t^{(j)}$ is 
...
\subsection{Simple mixture model}
...
\subsection{Localization with Hippocampal Place Cells}
...
\section{Discovering the Probabilistic Model from Spike Trains}

\TODO{Idea use an independent Bernoulli model for the adjacency
  matrix and a stochastic block model for the weights. Show that
  block assignments correspond to variable-value pairs and that
  the mean weights reflect the factor strengths.}
